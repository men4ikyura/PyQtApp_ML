from PyQt6.QtWidgets import QWidget, QHBoxLayout, QVBoxLayout,  QPushButton, QLabel, QApplication
...
class FinishImageWindow(QWidget):
    ...
    def center(self):
        ...
        # Получаем размеры окна (после отрисовки корректных размеров окна)
        self.show()  # Убедимся, что окно отображено и имеет корректные размеры
        window_size = self.frameGeometry()
        width_window = window_size.width()
        height_window = window_size.height()
        
        # Вычисляем координаты верхнего левого угла для центрирования
...
if __name__ == "__main__":
    app = QApplication([])
    window = FinishImageWindow(info_drops=[163, [[12, 12], 12]], file_path="/Users/yurazhilin/Desktop/pyQtApp/30YtrYE0stU.jpg")
    window.show()
    app.exec()